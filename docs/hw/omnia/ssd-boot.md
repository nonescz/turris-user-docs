...
!!! warning
    ...
    read our [community documentation](https://doc.turris.cz/doc/en/howto/omnia_booting_from_external_storage).
...
3. Check whether the new SSD can be detected: `ls /dev/sd*` It should display `/dev/sda`.
...
cfdisk /dev/sda
```
5. If no partition table exists it asks for a new one (its type should be `DOS`).
